...
class WC_Bsale_Admin_Hooks {
	...
	public function wc_bsale_product_edit_hook(): void {
		...
		if ( ! $product->managing_stock() ) {
			$message = esc_html__( 'This product is not marked with the "Manage stock" option. If you would like to sync it\'s stock with Bsale, please enable this option.', 'wc-bsale' );
		...
		if ( empty( $sku ) ) {
			$message = esc_html__( 'This product has no SKU. If you would like to sync it\'s stock with Bsale, please add a SKU to it.', 'wc-bsale' );
			$this->show_admin_notice( $message );
		...
		$wc_stock    = get_post_meta( $product->get_id(), '_stock', true );
		$bsale_api   = new WC_Bsale_API();
		$bsale_stock = $bsale_api->get_stock_by_code( $sku );

		// If the user has clicked the "Sync Stock" button, we update the product's stock with the stock in Bsale
		if ( isset( $_POST['wc_bsale_sync_stock'] ) ) {
			update_post_meta( $product->get_id(), '_stock', $bsale_stock );
